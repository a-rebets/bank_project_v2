...
class BaseForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        dates = {'data_registracii', 'data_posledney_operacii','data_opredelenia_statusa','data_resheniya_ob_osvobozhdenii', 'data_otkaza_v_osvobozhdenii_ot_raskritiya', 'data_proverki', 'data_zaprosa_po_neraskritiyu_informacii', 'data_predpisaniya_po_neraskritiyu_informacii', 'data_zaprosa_o_rezultatah_provedeniya_torgov_v_obchestve', 'data_otveta_na_zaprosa_o_rezultatah_provedeniya_torgov_v_obchestve', 'data_protokola','data_postanovleniya','data_pisma_v_fns','vzaimodeystvie_s_fns_na_ezhekvartalnoi_osnove_data_pisma'}
        placeholders = {
            'kolichestvo_licevyh_schetov_v_reestre': 'Нет',
            'kolichestvo_nominalnyh_derzhateley_v_reestre': 'Нет'
        }
        areas = {
            'zadolzhennost_pered_fns': '3',
            'otchetnost': '2'
        }
        possible = {
            'pao_v_silu_priznakov_st30': 'ДА',
            'pao_v_silu_priznakov_bez_st30': 'ДА',
            'pao_v_silu_nazvaniya_st30': 'ДА',
            'pao_v_silu_nazvaniya_bez_st30': 'ДА',
            'nao_bez_st30':'ДА',
            'nao_so_st30': 'ДА',
            'otkaz_v_osvobozhdenii_ot_raskritiya': 'ДА',
            'otkaz_v_registracii_vipuska': 'ОТКАЗ',
            'svedeniya_ob_adrese_ne_dostoverny': 'СВЕДЕНИЯ ОБ АДРЕСЕ НЕДОСТОВЕРНЫ'
        }
        super(BaseForm, self).__init__(*args, **kwargs)
        tmp = {}
        for field_name, field in self.fields.items():
            tmp['class'] = 'mdc-text-field__input'
            tmp['readonly'] = 'readonly'
            if field_name in placeholders:
                tmp['placeholder'] = placeholders[field_name]
            else:
                tmp['placeholder'] = 'Нет информации'
            if field_name in possible:
                tmp['possible'] = possible[field_name]
            if field_name in dates:
                tmp['placeholder'] = 'ДД.ММ.ГГГГ'
                tmp['data-toggle'] = 'datepicker'
            if field_name in areas:
                tmp['rows'] = areas[field_name]
                field.widget = forms.Textarea(attrs=tmp)
            else:
                field.widget = forms.TextInput(attrs=tmp)
            tmp = {}
            
class Data111Edit_KartochkaForm(BaseForm):
    class Meta:
        model = Data111
...
class Data111Edit_AdministrativkaForm(BaseForm):
    class Meta:
        model = Data111
...
class Data111Edit_Korp_KontrolForm(forms.ModelForm):
    ...
    registrator1 = forms.ChoiceField(required = False, label = '', choices = registrator_choices, widget=forms.RadioSelect)
    ...
    class Meta(object):
          ...
          fields = ['registrator1', "data_predpisanya_o_predstavlenii_documentov", "nomer_predpisanya_o_predstavlenii_documentov","data_zaprosa_po_reestru",
                     ...
                     "nomer_predpisaniya_po_1_vypusku", "nrd", "oao_na_22_06_2015","netsami","naimenovanie_registratora","vozvrat", "cod_emitenta", "nomer_predpisanya_o_predstavlenii_documentov_gosa",
                     "data_predpisanya_o_predstavlenii_documentov_gosa"]
...
class Data111Edit_RaskrytieForm(BaseForm):
    class Meta(object):
        model = Data111
...
class Data111FormSelection(forms.ModelForm):
    ...
    vid_deyatelnosti = forms.BooleanField(label = 'ВИД ДЕЯТЕЛЬНОСТИ', required=False)

    data_predpisanya_o_predstavlenii_documentov_gosa = forms.BooleanField(label = 'ДАТА ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ ГОСА', required=False)
    nomer_predpisanya_o_predstavlenii_documentov_gosa = forms.BooleanField(label = 'НОМЕР ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ ГОСА', required=False)

    data_predpisanya_o_predstavlenii_documentov_raskritie = forms.BooleanField(label = 'ДАТА ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ', required=False)
    nomer_predpisanya_o_predstavlenii_documentov_raskritie = forms.BooleanField(label = 'НОМЕР ПРЕДПИСАНИЯ О ПРЕДСТАВЛЕНИИ ДОКУМЕНТОВ', required=False)
    ...
    class Meta:
        ...
        fields = ['proverky_gosa_po_zaprosu','proverky_gosa_po_raskritiyu','naimenovanie', 'inn', 'ogrn','kpp','data_registracii','opf','cod_emitenta','ustavnoy_capital', 
                  ...
                  'data_zaclucheniya_uristam','nomer_zaclucheniya_uristam','vid_deyatelnosti','data_predpisanya_o_predstavlenii_documentov_gosa','nomer_predpisanya_o_predstavlenii_documentov_gosa',
                  'data_predpisanya_o_predstavlenii_documentov_raskritie','nomer_predpisanya_o_predstavlenii_documentov_raskritie']
        labels = {
            ...
            "vid_deyatelnosti": "Вид деятельности",
            "data_predpisanya_o_predstavlenii_documentov_gosa": "Дата предписания о представлении документов ГОСА",
            "nomer_predpisanya_o_predstavlenii_documentov_gosa": "Номер предписания о представлении документов ГОСА",
            "data_predpisanya_o_predstavlenii_documentov_raskritie": "Дата предписания о представлении документов",
            "nomer_predpisanya_o_predstavlenii_documentov_raskritie": "Номер предписания о представлении документов"
        }
...
class Data111_second_korp(forms.ModelForm):
    ...
    srok_dla_inspolnenia_1_choices = (('-','-'),('30', '30'), ('60','60'), ('90','90'))

    ispolneno_2 = forms.ChoiceField(required = False, label = 'Исполнено:', choices = ispolneno_2_choices, widget=forms.RadioSelect)
    ispolneno_1_2 = forms.ChoiceField(required = False, label = 'Исполнено:', choices = ispolneno_2_choices, widget=forms.RadioSelect)
    ispolneno_1_3 = forms.ChoiceField(required = False, label = 'Исполнено:', choices = ispolneno_2_choices, widget=forms.RadioSelect)
    ispolneno_3_3 = forms.ChoiceField(required = False, label = 'Исполнено:', choices = ispolneno_2_choices, widget=forms.RadioSelect)


    #1
    zapros_o_predostavlenii_informacii = forms.BooleanField(label="", required=False, widget=forms.CheckboxInput(attrs ={}))
    predpisanie_ob_istrebovanii_documentov = forms.BooleanField(label="", required=False,widget=forms.CheckboxInput(attrs ={}))
    prepsanie_ob_ustranenii_narusheniya_zakonodatelstva_RF = forms.BooleanField(label="", required=False,widget=forms.CheckboxInput(attrs ={}))
    informaciaya_o_gosah_s_saitov_raskritiya_informacii = forms.BooleanField(label="", required=False, widget=forms.CheckboxInput(attrs ={}))
    zapros_o_predostavlenii_informacii_3_2 = forms.BooleanField(label="", required=False, widget=forms.CheckboxInput(attrs ={}))
    predpisanie_ob_istrebovanii_documentov_3_3 = forms.BooleanField(label="", required=False, widget=forms.CheckboxInput(attrs ={}))
    ...
    srok_dla_inspolnenia_1 = forms.ChoiceField(label="", required=False, choices = srok_dla_inspolnenia_1_choices , widget=forms.Select())

    srok_dla_inspolnenia_1_3 = forms.ChoiceField(label="", required=False, choices = srok_dla_inspolnenia_1_choices , widget=forms.Select())

    ispolneno_1 = forms.ChoiceField(required = False, label = 'Исполнено:', choices = ispolneno_2_choices, widget=forms.RadioSelect)

    #2

    srok_dla_inspolnenia_2 = forms.ChoiceField(label="Срок для исполнения", required=False, choices = srok_dla_inspolnenia_1_choices , widget=forms.Select())
    
    srok_dla_inspolnenia_3_3 = forms.ChoiceField(label="Срок для исполнения", required=False, choices = srok_dla_inspolnenia_1_choices , widget=forms.Select())
    


    class Meta:
        model = Data_korp_kontrol
        fields = ['data_proverki', "polucheno_2", "srok_dla_inspolnenia_2", "data_instechenia_sroka_inpolnenia_2", "dokumenti_predstavleni_2", 
        "ispolneno_2",'zapros_o_predostavlenii_informacii','predpisanie_ob_istrebovanii_documentov','prepsanie_ob_ustranenii_narusheniya_zakonodatelstva_RF',
         'polucheno_1_2_bool','polucheno_1_3_bool','polucheno_1_2','polucheno_1_3', 'srok_dla_inspolnenia_1', 'data_instechenia_sroka_inpolnenia_1',
          "otvet_1_2", 'data_otveta_1_2', "ispolneno_1", "data_provedinia_3_1_1", "data_sostavlenia_spiska_lich_imeushih_pravo_na_uchastie_v_gosa",
          "data_sostavlenia_protokola_gosa", "povestka_dnia_gosa", "sroki_provedenia_gosa_po_ustavu", "informaciaya_o_gosah_s_saitov_raskritiya_informacii", "srok_dla_inspolnenia_1_3",
          'data_instechenia_sroka_inpolnenia_1_3',"otvet_1_3", "data_otveta_1_3", "ispolneno_1_2", "ispolneno_1_3", "zapros_o_predostavlenii_informacii_3_2",
          "predpisanie_ob_istrebovanii_documentov_3_3", "polucheno_3_3", "srok_dla_inspolnenia_3_3" , "data_instechenia_sroka_inpolnenia_3_3" ,
          "otvet_3_3", "data_otveta_3_3", "ispolneno_3_3"]
        labels = {
            "data_proverki": "Дата проверки",
            "polucheno_2": "Получено",
            "polucheno_3_3": "Получено",
            "srok_dla_inspolnenia_2": "Срок для исполнения",
            "srok_dla_inspolnenia_3_3": "Срок для исполнения",
            "data_instechenia_sroka_inpolnenia_2": "Дата истечения срока исполнения",
            "data_instechenia_sroka_inpolnenia_3_3": "Дата истечения срока исполнения",
            "dokumenti_predstavleni_2":"Документы представлены",
            "ispolneno_2": "Исполнено",
            "ispolneno_1_2": "Исполнено",
            "ispolneno_1_3": "Исполнено",
            "ispolneno_3_3": "Исполнено",
            'zapros_o_predostavlenii_informacii':'',
            'polucheno_1_2_bool':'Получено:',
            'polucheno_1_3_bool':'Получено:',
            'polucheno_1_2':'',
            'polucheno_1_3':'',
            "informaciaya_o_gosah_s_saitov_raskritiya_informacii":'',
            "zapros_o_predostavlenii_informacii_3_2": '',
            "predpisanie_ob_istrebovanii_documentov_3_3": '',
            'srok_dla_inspolnenia_1': "Срок для исполнения",
            'data_instechenia_sroka_inpolnenia_1': "Дата истечения срока исполнения",
            "data_instechenia_sroka_inpolnenia_1_3": "Дата истечения срока исполнения",
            'otvet_1_2': '',
            "otvet_1_3": '',
            'data_otveta_1_2':'',
            "data_otveta_1_3": '',
            "otvet_3_3": "",
            "data_otveta_3_3": "",
            "data_provedinia_3_1_1": "Дата проведения заседания Совета директоров или иного органа управления, на котором приянто решение о созыве ГОСА",
            "data_sostavlenia_spiska_lich_imeushih_pravo_na_uchastie_v_gosa": "Дата составления списка лиц, имеющих право на участие в ГОСА",
            "data_sostavlenia_protokola_gosa": "Дата составления протокола ГОСА",
            "povestka_dnia_gosa": "Повестка дня ГОСА",
            "sroki_provedenia_gosa_po_ustavu": "Сроки проведения ГОСА по Уставу",
            "srok_dla_inspolnenia_1_3": "Срок для исполнения",
